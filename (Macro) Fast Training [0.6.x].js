// This returns an array containing all the activity objects from Crash's 5e Downtime Tracking.
...
function printButtonSkill(id, label, abbr) {
    ...
    this.callback = function (html) {
        ...
        let vTwenties = roll.dice[0].rolls.filter(d => d.roll === 20).length;
        let vOnes = roll.dice[0].rolls.filter(d => d.roll === 1).length;
        ...
        if (!rb) {
            ...
            let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 15 - token.actor.data.data.skills[abbr].mod).length;
            let vTotal = (vTwenties + vSuccess) - vOnes;
            let vFailures = hours - vSuccess;
            let message = `<H2>DOWNTIME REPORT</H2>
                <P><B>Successes: </B>${vSuccess}</P>
                <P><B>Failures: </B>${vFailures}</P>
                <P><B>Nat. 20s: </B>${vTwenties}</P>
                <P><B>Nat. 1s: </B>${vOnes}</P>
                <P><B>Total: </B>${vTotal}</P>` 
        message += `<P><B>${actor.name}</B> trains <I>${label}</I> hard for ${hours} hours! PUSH IT TO THE LIMIT!</P>`
        printMessage(message);
        } else {
            let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 20 - token.actor.data.data.skills[abbr].mod).length;
            let vTotal = (vTwenties + vSuccess) - vOnes;
            let vFailures = hours - vSuccess;
            let message = `<H2>DOWNTIME REPORT</H2>
                <P><B>Successes: </B>${vSuccess}</P>
    ...
    this.callback = function (html) {
        ...
        if (rb == "prof") {
            if (!rb) {
                ...
                let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 15 - token.actor.data.data.abilities.dex.mod).length;
                let vTotal = (vTwenties + vSuccess) - vOnes;
                let vFailures = hours - vSuccess;
                let message = `<H2>DOWNTIME REPORT</H2>
                    <P><B>Successes: </B>${vSuccess}</P>
                ...
                let roll = new Roll(`${hours}(d20 + @abilities.str.mod)`, token.actor.getRollData()).roll()
                let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 15 - token.actor.data.data.abilities.str.mod).length;
                let vTotal = (vTwenties + vSuccess) - vOnes;
                let vFailures = hours - vSuccess;
                let message = `<H2>DOWNTIME REPORT</H2>
                    <P><B>Successes: </B>${vSuccess}</P>
                ...
                let roll = new Roll(`${hours}d20 + (@abilities.str.mod + @abilities.dex.mod) / 2`, token.actor.getRollData()).roll().total;
                let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 15 - (token.actor.data.data.abilities.dex.mod + token.actor.data.data.abilities.str.mod) / 2).length;
                let vTotal = (vTwenties + vSuccess) - vOnes;
                let vFailures = hours - vSuccess;
                let message = `<H2>DOWNTIME REPORT</H2>
                    <P><B>Successes: </B>${vSuccess}</P>
            ...
            if (!rb) {
                ...
                let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 20 - token.actor.data.data.abilities.dex.mod).length;
                let vTotal = (vTwenties + vSuccess) - vOnes;
                let vFailures = hours - vSuccess;
                let message = `<H2>DOWNTIME REPORT</H2>
                    <P><B>Successes: </B>${vSuccess}</P>
                ...
                let roll = new Roll(`${hours}(d20 + @abilities.str.mod)`, token.actor.getRollData()).roll()
                let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 20 - token.actor.data.data.abilities.str.mod).length;
                let vTotal = (vTwenties + vSuccess) - vOnes;
                let vFailures = hours - vSuccess;
                let message = `<H2>DOWNTIME REPORT</H2>
                    <P><B>Successes: </B>${vSuccess}</P>
                ...
                let roll = new Roll(`${hours}d20 + (@abilities.str.mod + @abilities.dex.mod) / 2`, token.actor.getRollData()).roll().total;
                let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 20 - (token.actor.data.data.abilities.dex.mod + token.actor.data.data.abilities.str.mod) / 2).length;
                let vTotal = (vTwenties + vSuccess) - vOnes;
                let vFailures = hours - vSuccess;
                let message = `<H2>DOWNTIME REPORT</H2>
                    <P><B>Successes: </B>${vSuccess}</P>
...
function printButtonLanguage(id, label) {
    ...
    this.callback = function (html) {
        ...
        if (hours == 0) {
            ...
            let vTwenties = roll.dice[0].rolls.filter(d => d.roll === 20).length;
            let vOnes = roll.dice[0].rolls.filter(d => d.roll === 1).length;
            let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 15 - token.actor.data.data.abilities.int.mod).length;
            let vTotal = (vTwenties + vSuccess) - vOnes;
            let vFailures = hours - vSuccess;
            let message = `<H2>DOWNTIME REPORT</H2>
                <P><B>Successes: </B>${vSuccess}</P>
...
function printButtonTool(id, label) {
    ...
    this.callback = function (html) {
        ...
        if (hours == 0) {
            ...
            let vTwenties = roll.dice[0].rolls.filter(d => d.roll === 20).length;
            let vOnes = roll.dice[0].rolls.filter(d => d.roll === 1).length;
            let vSuccess = roll.dice[0].rolls.filter(d => d.roll >= 20 - token.actor.data.data.abilities.int.mod).length;
            let vTotal = (vTwenties + vSuccess) - vOnes;
            let vFailures = hours - vSuccess;
            let message = `<H2>DOWNTIME REPORT</H2>
                <P><B>Successes: </B>${vSuccess}</P>
